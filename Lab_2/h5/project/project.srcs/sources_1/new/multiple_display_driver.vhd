...
begin
    decoder : entity work.bcd_to_segments(gedrag)
    port map(
                data => data_internal,
                segments => segments
            );
    ...
    sw_multiplexer : process(data_selector(15 downto 14), display_data, internal_display_mask)

    begin

        case data_selector(15 downto 14) is
            when "00" =>
                data_internal <= display_data(3 downto 0);
                internal_display_selector <= "1110";
            when "01" =>
                data_internal <= display_data(7 downto 4);
                internal_display_selector <= "1101";
            when "10" =>
                data_internal <= display_data(11 downto 8);
                internal_display_selector <= "1011";
            when "11" =>
                data_internal <= display_data(15 downto 12);
    ...
    anodes <= internal_display_mask or internal_display_selector;
